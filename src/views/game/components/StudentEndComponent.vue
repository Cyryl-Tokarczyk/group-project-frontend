...
onMounted(() => {
    if (props.message['winner']  == "student"){
      win.value = true;
    }
    else if (props.message['winner']  == "teacher"){
      win.value = false;
    }
    else{
      win.value = true;
    ...
    setTimeout(()=>{
      test.value.classList.remove('animate_slide_in');
    },250)
});
</script>
...
<template>
  <div id="student_end">
    <div class="test_paper" ref="test">
      ...
      <span class="black_color">{{ win ? "I have triumphed over the teacher, earning an A+ grade." : "Better luck next time! The teacher has triumphed over me, earning an F grade." }}</span>
